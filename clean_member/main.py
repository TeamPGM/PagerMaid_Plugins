...
async def check_self_and_from(message: Message):
    ...
    try:
        ...
        # 检查消息发送者是否为管理员
        sender_participant = await bot.get_permissions(message.chat_id, message.sender_id)
...
async def kick_chat_member(cid, uid, only_search: bool = False):
    ...
    if only_search:
        return
    try:
        with contextlib.suppress(UserAdminInvalidError, PeerIdInvalidError, BadRequestError):
...
🔧 **多重获取方法:** aggressive + 多过滤器 + 搜索""")
        ...
        participants = await asyncio.wait_for(
            get_all_participants_advanced(message.chat_id, 50000), 
            timeout=300
        )
...
🎯 **开始筛选符合条件的用户...**""")
        ...
        admin_ids = set()
        try:
            async for admin in bot.iter_participants(message.chat_id, filter=lambda p: hasattr(p, 'participant') and isinstance(p.participant, (ChannelParticipantCreator, ChannelParticipantAdmin))):
                admin_ids.add(admin.id)
        except:
            pass
        
        # 筛选目标用户
        target_users = await filter_target_users(participants, message.chat_id, mode, day, admin_ids)
        
        if only_search:
            # 保存到缓存
            cache_file = await save_cache(message.chat_id, mode, day, target_users, chat_title)
...
async def clean_member(message: Message):
    ...
    if mode == "1":
        ...
        return await message.edit("❌ **模式错误**\n\n请输入有效的模式(1-5)\n使用 `-clean_member` 查看帮助")
