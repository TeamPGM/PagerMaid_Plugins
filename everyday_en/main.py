...
async def everyday_en(message: Message):
    """每日英语"""
    if not message.arguments:
        ...
        await push_everyday_en(message.chat_id)
    elif message.arguments == "订阅":
        if from_self(message) or enforce_permission(
            from_msg_get_sudo_uid(message), "modules.manage_subs"
        ):
            if everyday_en_sub.check_id(message.chat_id):
                return await edit_delete(message, "❌ 你已经订阅了每日英语")
            everyday_en_sub.add_id(message.chat_id)
            await message.edit("你已经成功订阅了每日英语")
        else:
            await edit_delete(message, "❌ 权限不足，无法订阅每日英语")
    elif message.arguments == "退订":
        if from_self(message) or enforce_permission(
            from_msg_get_sudo_uid(message), "modules.manage_subs"
        ):
            if not everyday_en_sub.check_id(message.chat_id):
                return await edit_delete(message, "❌ 你还没有订阅每日英语")
            everyday_en_sub.del_id(message.chat_id)
            await message.edit("你已经成功退订了每日英语")
        else:
            await edit_delete(message, "❌ 权限不足，无法退订每日英语")
